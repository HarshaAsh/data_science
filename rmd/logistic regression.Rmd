...
This post can be found at [www.harshaash.website](http://www.harshaash.website/logistic-regression/)
...
### Model Diagnostics
The created model can be validated using various tests such as the Omnibus test, Wald's test, Hosmer-Lemeshow's test etc. The assumptions in these hypothesis tests can be validated thru residual plots, and finally I want to check for multicolliniarity and Pseudo R square.  
...
Some of the assumptions taken during the hypothesis tests are:  
1. The mean of errors is zero  
2. The errors have constant variance  
3. The errors follow a normal distribution  
These assumptions can be visually validated using residual plots  
...
Like in case of linear regression, we should check for multicollinearity in the model. Multicollinearity is given by VIF. VIF above 4 means there is significant multicollinearity.   
```{r vif, echo=TRUE}
library(car)
...
2. Youden's Index  
Youdens index can be used to find cutoff when sensitivity and specificity are equally important. It is the point which has minimum distance from ROC curve.  
