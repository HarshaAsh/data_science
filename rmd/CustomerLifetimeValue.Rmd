...
library(readxl)
library(lubridate)
raw_data <- read_excel('C:\\Users\\achyuthuni.harsha\\Downloads/Online Retail.xlsx')
```
...
The customer segments can be represented as states of the Markov chain. Let {0, 1, 2, â¦, m} be the states of a Markov chain in which states {1, 2,..., m} denote different customer segments and state 0 denotes non-customer state.  
...
$$ CLT = \sum_{t=0}^{N} \frac{P_IÃP^tÃR}{(1+i)^t} $$
...
But before we can create the one-step transition probabilities, we need to check whether the sequence of random variables form a Markov chain. This is carried out using Andersonâ Goodman test which is a chi-square test of independence.   
...
H0: The sequences of transitions (X1, X2, â¦, Xn) are independent (zero-order Markov chain)   
HA: The sequences of transitions (X1, X2, â¦, Xn) are dependent (first-order Markov chain)  
...
As the p_value is 0.8, rejecting the Null Hypothesis. The data can be assumed as a first order markov chain.  
...
seq_matr$estimate
```
From the above matrix, P(4,5) = 0.8684211 means that the probability of moving from State 4 to State 5 is 84%. That means that a customer that has not purchased any item in 3 months has a 84% probability of not purchasing any item the next month also.  
...
The Transition Probabilities at various steps seem to follow a similar pattern. We can do a likelihood ratio test to test the homogeneity of transition matrices. That means I want to find out if the changes in TP across time are random and I can take a constant TP to describe the different TP;s or not.  
...
where n(t) is the number of customers in state i at time t. The test statistic follows a $\chi^2$ distribution with (t â 1) Ã m Ã (m â 1) degrees of freedom.  
...
As TP is the transition probability between states, we can find the number of people in each state at any time **t** using the following formulae:

$$u^{(n)} = uP^{(n)}$$

From the TP, the frequency in the first stage is compared with the actual frequency at the 1nd stage. 
```{r}
final_classes %>% group_by(Class2) %>% summarise(actual_frequency = n()/nrow(final_classes)) %>% 
  mutate(predicted_frequency = initial_freq*CLT.mc)
```

Similarly for 3rd stage
```{r}
final_classes %>% group_by(Class4) %>% summarise(actual_frequency = n()/nrow(final_classes)) %>% 
  mutate(predicted_frequency = initial_freq*CLT.mc^3)
```
