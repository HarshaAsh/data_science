...
```{r}
gini_entropy_comparision <- data.frame(p = (1:99)/100) %>% mutate(gini.index = p*(1-p), entropy = -p*log(p) -(1-p)*log(1-p))
...
gini.index <- prob_list*(1-prob_list)
print(as.numeric(gini.index))
```

The impurity of the overall data is 0.236. Let us look at the purity if we split the data based on various features.  
...
#### Conclusion
Among all the factors above, Gender has the least Gini Index of 0.16. So the initial split of the data would be on Gender.  
